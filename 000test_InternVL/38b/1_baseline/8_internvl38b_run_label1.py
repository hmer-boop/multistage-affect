...
TXT_OUT = OUT_DIR / "results_stage1_internvl38b.txt"
JSON_OUT = OUT_DIR / "results_stage1_internvl38b.json"
REPORT_PATH = OUT_DIR / "missing_report_stage1_internvl38b.json"
LOCK_PATH = OUT_DIR / ".results_stage1_internvl38b.lock"

TXT_OUT_ROUND2 = OUT_DIR / "results_stage1_internvl38b_round2.txt"
JSON_OUT_ROUND2 = OUT_DIR / "results_stage1_internvl38b_round2.json"
REPORT_PATH_ROUND2 = OUT_DIR / "missing_report_stage1_internvl38b_round2.json"
LOCK_PATH_ROUND2 = OUT_DIR / ".results_stage1_internvl38b_round2.lock"

# 38B 先以稳为主；若 endpoint 足够稳定再尝试改成 2
MAX_WORKERS = 1
RATE_DELAY = 0.0
...
# ========== HF dataset 配置（已切到新版 512） ==========
HF_DATASET_REPO = "hmer123/affectecom_512"
HF_DATASET_REV = "main"

# 本地 cloud 图目录 / manifest（已切到新版 512）
LOCAL_CLOUD_IMAGE_DIR = PROJECT_ROOT / "dataset_hf_upload" / "images_cloud_512"
CLOUD_MANIFEST_PATH = PROJECT_ROOT / "dataset_hf_upload" / "metadata_cloud_512" / "images_cloud_512_manifest.json"
...
def _extract_rel_from_string(s: str, cloud_root: Path) -> Optional[str]:
    ...
    marker = "/images_cloud_512/"
    if marker in s:
        return s.split(marker, 1)[1]

    if s.startswith("images_cloud_512/"):
        return s[len("images_cloud_512/"):]
...
def build_raw_to_cloud_repo_path(manifest_path: Path, local_cloud_dir: Path) -> Dict[str, str]:
    ...
    if isinstance(obj, dict):
        for raw_path, payload in obj.items():
            ...
            if rel:
                mapping[src_abs] = f"images_cloud_512/{rel}"
...
def worker_stage1(item: Dict, client: "InferenceClient", rate_delay: float) -> Dict:
...
def run_one_dataset(
    ...
    print(f"[index:{dataset_name}] stage1 输入条数: {len(items)} | unique ids: {len(all_ids)}")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futures = [
            ex.submit(worker_stage1, it, client, RATE_DELAY)
            for it in todo
        ]
            ...
            try:
                ...
                if SKIP_ON_TIMEOUT:
                    ...
                    if FAIL_FAST_ON_ERROR:
                        raise RuntimeError(f"[{dataset_name}] Stage1 测试阶段发生超时，已主动停止，避免继续烧钱。")
                    continue
                raise
            except Exception as e:
                print(f"[error:{dataset_name}] worker: {e}")
                if FAIL_FAST_ON_ERROR:
                    raise RuntimeError(f"[{dataset_name}] Stage1 在首个错误处已中止，避免继续烧钱。原始错误: {e}")
...
def main():
    ...
    if TEST_LIMIT > 0:
        ...
        print(f"[test] 当前为测试模式，仅跑 stage1 base 前 {TEST_LIMIT} 张。将不会跑 round2。")
