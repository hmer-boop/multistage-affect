...
OUT_DIR = SCRIPT_DIR / "result internvl_stage2"
OUT_DIR.mkdir(parents=True, exist_ok=True)

TXT_OUT = OUT_DIR / "results_stage2_internvl38b.txt"
JSON_OUT = OUT_DIR / "results_stage2_internvl38b.json"
REPORT_PATH = OUT_DIR / "missing_report_stage2_internvl38b.json"
LOCK_PATH = OUT_DIR / ".results_stage2_internvl38b.lock"

TXT_OUT_ROUND2 = OUT_DIR / "results_stage2_internvl38b_round2.txt"
JSON_OUT_ROUND2 = OUT_DIR / "results_stage2_internvl38b_round2.json"
REPORT_PATH_ROUND2 = OUT_DIR / "missing_report_stage2_internvl38b_round2.json"
LOCK_PATH_ROUND2 = OUT_DIR / ".results_stage2_internvl38b_round2.lock"
...
def worker_stage2(item: Dict, client: "InferenceClient",
                  rate_delay: float) -> Dict:
    iid = item.get("item_id")
    path_raw = item.get("path_raw")

    image_url = raw_path_to_hf_url(path_raw)
    out = call_vision(client, prompt_stage2_text(), image_url)
            ...
            try:
                ...
                if SKIP_ON_TIMEOUT:
                    ...
                    if FAIL_FAST_ON_ERROR:
                        raise RuntimeError(f"[{dataset_name}] Stage2 测试阶段发生超时，已主动停止，避免继续烧钱。")
                    continue
                raise
            except Exception as e:
                print(f"[error:{dataset_name}] worker: {e}")
                if FAIL_FAST_ON_ERROR:
                    raise RuntimeError(f"[{dataset_name}] Stage2 在首个错误处已中止，避免继续烧钱。原始错误: {e}")
...
def main():
    ...
    if TEST_LIMIT > 0:
        ...
        print(f"[test] 当前为测试模式，仅跑 stage2 base 前 {TEST_LIMIT} 张。将不会跑 round2。")
