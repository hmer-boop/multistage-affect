...
# COT 输出文件中可能出现的标签字段名。
# 优先读取 stage1/stage1_label 等阶段字段；如果没有，再读取通用字段 label/prediction 等。
GENERIC_LABEL_KEYS = [
...
def build_common_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument('--project-root', default=str(DEFAULT_PROJECT_ROOT), help='项目根目录，默认是仓库根目录')
    ...
    parser.add_argument('--pred-stage1-round2', default=None, help='stage1 round2 预测文件路径，可选')
    parser.add_argument('--pred-stage2', default=None, help='stage2 预测文件路径，或其所在目录')
    parser.add_argument('--pred-stage2-round2', default=None, help='stage2 round2 预测文件路径，可选')
    parser.add_argument('--pred-stage3', default=None, help='stage3 预测文件路径，或其所在目录')
    parser.add_argument('--pred-stage3-round2', default=None, help='stage3 round2 预测文件路径，可选')
    parser.add_argument('--quiet-paths', action='store_true', help='不打印本次实际读取的金标/预测路径')
    return parser
...
def resolve_common_args(args: argparse.Namespace) -> MetricIO:
    ...
    return MetricIO(
        ...
        # 下面只是“单指标脚本单独运行且未传路径”时的兜底。
        # 正常请通过 run_eval_center_cot.py 统一传入明确路径。
        pred_stage1=_resolve_stage_path(args.pred_stage1, project_root, '000 cot/results/gpt4o/stage1', 'stage1'),
        pred_stage1_round2=_resolve_stage_path(args.pred_stage1_round2, project_root, '000 cot/results/gpt4o/stage1', 'stage1') if args.pred_stage1_round2 else None,
        pred_stage2=_resolve_stage_path(args.pred_stage2, project_root, '000 cot/results/gpt4o/stage2', 'stage2'),
        pred_stage2_round2=_resolve_stage_path(args.pred_stage2_round2, project_root, '000 cot/results/gpt4o/stage2', 'stage2') if args.pred_stage2_round2 else None,
        pred_stage3=_resolve_stage_path(args.pred_stage3, project_root, '000 cot/results/gpt4o/stage3', 'stage3'),
        pred_stage3_round2=_resolve_stage_path(args.pred_stage3_round2, project_root, '000 cot/results/gpt4o/stage3', 'stage3') if args.pred_stage3_round2 else None,
    )


def print_metric_io(io: MetricIO, quiet: bool = False) -> None:
    if quiet:
        return
    print('----- 本次实际读取路径 -----')
    print(f'PROJECT_ROOT : {io.project_root}')
    print(f'gold_base    : {io.gold_base}')
    print(f'gold_round2  : {io.gold_round2}')
    print(f'pred_stage1  : {io.pred_stage1}')
    if io.pred_stage1_round2:
        print(f'pred_stage1_r2: {io.pred_stage1_round2}')
    print(f'pred_stage2  : {io.pred_stage2}')
    if io.pred_stage2_round2:
        print(f'pred_stage2_r2: {io.pred_stage2_round2}')
    print(f'pred_stage3  : {io.pred_stage3}')
    if io.pred_stage3_round2:
        print(f'pred_stage3_r2: {io.pred_stage3_round2}')
    print('----------------------------')
...
def merge_pred_maps(base: Dict[str, Any], extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    merged = dict(base)
    for item_id, rec in (extra or {}).items():
...
def rows_from_obj(obj: Any, path: Path) -> List[dict]:
    if isinstance(obj, dict) and isinstance(obj.get('valid'), list):
        ...
        raise ValueError(f'{path.name} 格式不符合预期：应为列表，或包含 valid/results/data 的字典。')
...
def split_labels(x: Any, splitters: List[str]) -> List[str]:
    ...
    if isinstance(x, dict):
        ...
        # 有些模型会把标签包在 {'label': '宁静'} 里
        for k in GENERIC_LABEL_KEYS:
            if k in x:
                return split_labels(x.get(k), splitters)
        return []
...
def extract_stage_label(record: Dict[str, Any], stage: str) -> Any:
    ...
    优先级：
    1. stage1 / stage1_label / stage1_pred / stage1_prediction 等阶段字段；
    2. 字段名包含 stage1 且包含 label/pred/emotion/result 的字段；
    3. label / prediction / emotion 等通用字段。
    """
    if not record:
        return None
    ...
    for key in stage_specific_keys:
        if key in record:
            return record[key]

    # 字段名里带 stage 的情况下，优先找真正像“标签”的字段，避免误读 reason/thought。
    for k, v in record.items():
        kl = str(k).lower()
        if stage in kl and any(tok in kl for tok in ['label', 'pred', 'emotion', 'result', 'answer']):
            return v

    for key in GENERIC_LABEL_KEYS:
        if key in record:
            return record[key]
