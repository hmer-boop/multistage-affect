...
def source_name_from_path(path_text: str) -> str:
    """兼容Windows 路径和当前 macOS 路径，提取源文件名。"""
...
def ensure_existing_bilingual_titles(rows, client, cache: dict) -> int:
    ...
    if not total:
        print("已有记录双语标题已完整，无需补齐。")
        return 0

    print(f"开始补齐已有记录双语标题：{total} 条")
    updated = 0
...
def main():
    ...
    if args.backfill_existing_titles:
        ...
        if updated_existing:
            ...
            print(f"已补齐已有记录双语标题 {updated_existing} 条；备份：{backup}")
    else:
        print("跳过已有记录双语标题补齐；本次只处理新增图片。")
