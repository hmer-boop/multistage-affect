# -*- coding: utf-8 -*-
"""
8_run_stage1.py —— Stage1 单阶段标注（先TXT落盘→再导出JSON→诊断）
- 输入:
  - dataset/metadata/items_stage1_2.json
  - dataset/metadata/items_stage1_2_round2.json
- 输出: result gpt_stage1/
  - results_stage1.txt
  - results_stage1.json
  - missing_report_stage1.json
  - results_stage1_round2.txt
  - results_stage1_round2.json
  - missing_report_stage1_round2.json

断点续跑：各自 TXT 中已有 stage1 的 item_id 将被跳过
"""
...
OUT_DIR = BASE_DIR / "result gpt_stage1"
...
def parse_args():
    ...
    ap.add_argument("--model", default="gpt-4o", help="支持视觉输入的模型（如 gpt-4o / gpt-4.1 等）")
