# -*- coding: utf-8 -*-
"""
8_run_stage3.py —— Stage3 单阶段标注（结合 title_en/caption_en）
- 输入:
  - dataset/metadata/items_stage3.json
  - dataset/metadata/items_stage3_round2.json
- 输出: result gpt_stage3/
  - results_stage3.txt
  - results_stage3.json
  - missing_report_stage3.json
  - results_stage3_round2.txt
  - results_stage3_round2.json
  - missing_report_stage3_round2.json

断点续跑：各自 TXT 中已有 stage3 的 item_id 将被跳过
"""
...
OUT_DIR = BASE_DIR / "result gpt_stage3"
...
def call_vision(client: OpenAI, model: str, instruction: str, image_data_url: str,
                title_en: Optional[str] = None, caption_en: Optional[str] = None,
                request_timeout: float = 30.0) -> str:
    content_blocks = [{"type": "input_text", "text": instruction}]
    if title_en is not None or caption_en is not None:
        txt = f"Title: {title_en or ''}\nCaption: {caption_en or ''}"
        content_blocks.append({"type": "input_text", "text": txt})
    content_blocks.append({"type": "input_image", "image_url": image_data_url})
...
def worker_stage3(item: Dict, client: OpenAI, model: str,
    ...
    out = call_vision(
        client, model, prompt_stage3_text(), data_url,
        title_en, caption_en,
...
def run_one_dataset(
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as ex:
        futures = [
            ex.submit(
                worker_stage3, it, client, args.model,
