...
TXT_OUT = OUT_DIR / "results_stage1_qwen8b.txt"
JSON_OUT = OUT_DIR / "results_stage1_qwen8b.json"
REPORT_PATH = OUT_DIR / "missing_report_stage1_qwen8b.json"
LOCK_PATH = OUT_DIR / ".results_stage1_qwen8b.lock"

TXT_OUT_ROUND2 = OUT_DIR / "results_stage1_qwen8b_round2.txt"
JSON_OUT_ROUND2 = OUT_DIR / "results_stage1_qwen8b_round2.json"
REPORT_PATH_ROUND2 = OUT_DIR / "missing_report_stage1_qwen8b_round2.json"
LOCK_PATH_ROUND2 = OUT_DIR / ".results_stage1_qwen8b_round2.lock"
...
def worker_stage1(item: Dict, client: OpenAI, model: str,
    ...
    out = call_vision(client, model, prompt_stage1_text(), data_url, request_timeout=request_timeout)
...
def parse_args():
    ...
    ap.add_argument("--model", default="qwen3-vl-8b-instruct", help="DashScope 视觉模型，如 qwen3-vl-8b-instruct / qwen3-vl-8b-thinking")
    ...
    ap.add_argument("--request_timeout", type=float, default=30.0, help="单次API请求超时（默认30s）")
    ap.add_argument("--future_timeout", type=float, default=120.0, help="每个样本总体等待上限（默认120s）")
    ap.add_argument("--skip_on_timeout", action="store_true", help="超时跳过该样本（不写空值，下次再补）")
    ap.add_argument("--flush_every", type=int, default=20, help="每 N 个结果强制落盘重写TXT（默认20）")
...
def run_one_dataset(
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as ex:
        futures = [
            ex.submit(
                worker_stage1, it, client, args.model,
