# -*- coding: utf-8 -*-
"""
8_Gemini25pro_run_label3.py —— Gemini 2.5 Pro Stage3 单阶段标注（并发提速版）

用途：
1. 将原 GPT baseline 的 label3 迁移到 Gemini 2.5 Pro。
2. 输出结构与分 stage 目录一致：当前脚本只写入
   src/inference/base/gemini/gemini_2_5_pro/result Gemini_stage3/
3. 支持断点续跑、并发处理、定期落盘、进度打印、round2 独立输出。

输入（项目根目录，不复制）：
- dataset/metadata/items_stage3.json
- dataset/metadata/items_stage3_round2.json

输出（当前脚本目录下的独立 stage3 文件夹）：
- result Gemini_stage3/results_stage3.txt
- result Gemini_stage3/results_stage3.json
- result Gemini_stage3/missing_report_stage3.json
- result Gemini_stage3/results_stage3_round2.txt
- result Gemini_stage3/results_stage3_round2.json
- result Gemini_stage3/missing_report_stage3_round2.json
"""
...
LOCK_PATH = OUT_DIR / ".results_stage3.lock"
...
LOCK_PATH_ROUND2 = OUT_DIR / ".results_stage3_round2.lock"
...
# worker
# =========================================================
def worker_stage3(item: Dict, args, api_key: str, types_module):
    ...
    if title_en is not None or caption_en is not None:
        contents.append(f"Title: {title_en or ''}\nCaption: {caption_en or ''}")
...
# =========================================================
def parse_args():
    ap = argparse.ArgumentParser(description="Gemini 2.5 Pro Stage3 labeling with TXT/JSON dual save")
    ...
    ap.add_argument("--max_workers", type=int, default=4, help="并发线程数（默认 4）")
...
def run_one_dataset(
    ...
    if not todo:
        ...
        def submit_with_retry(executor, item):
            return executor.submit(worker_stage3, item, args, api_key, types_module)
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as ex:
            ...
            while future_to_ctx:
                for fut in as_completed(list(future_to_ctx.keys())):
                    ...
                    try:
                        res = fut.result()
                        existing[res["item_id"]] = res["stage3"]
                        new_count += 1

                        print(
                            f"[stage3:{dataset_name}] {new_count}/{total_todo} | {res['item_id']} -> 原始:{res['raw_result']} | 归一化:{res['stage3']}"
                        )
