# -*- coding: utf-8 -*-
"""
8.1_Gemini25pro_run_label1_improve_batch.py

Gemini 2.5 Pro Stage1 improve 批处理版（RPD 优化）。

设计目标
1) 保留原 stage1 improve 的核心输入：本地图片 + stage1_cues 的 5 条全局 cues。
2) 不重新跑证据抽取，直接复用已有 stage1_cues.json。
3) 改为 batch 模式：每次请求处理多张图（默认 10 张），以减少每日请求次数。
4) 使用本地 512 图像目录作为模型输入图片来源。
5) 优先复用 Gemini Files API：同一张 512 图像上传一次，后续批次复用。
6) 自动生成 master / batch manifests，并支持断点续跑。
7) 自动读取已有单图版 stage1 improve 结果，已完成样本默认跳过不重跑。
8) 已有结果会直接合并复制到本次 batch 输出目录的 merged 结果文件中，便于后续只看这一套文件与金标对照。

建议放置目录：
src/inference/improved/gemini/gemini_2_5_pro/batch/
"""
...
PROMPT_VERSION = "stage1_glance_v1_batch"
...
MERGED_JSON_PATH = OUT_DIR / "stage1_gemini_labels4.json"
REPORT_PATH = OUT_DIR / "stage1_gemini_labels4_missing_report.json"
BATCH_OUTPUT_DIR = OUT_DIR / "batch_outputs"
MASTER_PATH = MANIFEST_MASTER_DIR / "stage1_improve_master.json"
BATCH_DIR = MANIFEST_BATCH_DIR / "stage1_improve"

LEGACY_RESULT_DIR_DEFAULT = BASE_DIR.parent.parent / "2_improve" / "result Gemini_stage1"
LEGACY_RESULT_JSON_DEFAULT = LEGACY_RESULT_DIR_DEFAULT / "stage1_gemini_labels4.json"
...
def merge_existing_records(primary_map: Dict[str, Dict[str, Any]], extra_records: List[Dict[str, Any]]) -> Tuple[Dict[str, Dict[str, Any]], int]:
    ...
    for rec in extra_records:
        ...
        existing_label = normalize_label(str((existing or {}).get("stage1_label", ""))) if existing else "未知"
        new_label = normalize_label(str(rec.get("stage1_label", "")))
        if existing is None:
            merged[iid] = rec
            added += 1
...
def build_done_id_set(records: Dict[str, Dict[str, Any]]) -> set[str]:
    ...
    for iid, r in records.items():
        label = normalize_label(str(r.get("stage1_label", "")))
...
def build_master_manifest(ids: List[str], cues_map: Dict[str, Dict[str, Any]], image_dir: Path) -> Dict[str, Any]:
    ...
    for item_id in ids:
        ...
        top5 = pick_stage1_5_cues(cues_map.get(item_id))
        rows.append(
            {
                "item_id": item_id,
...
STAGE1_IMPROVE_BATCH_RESPONSE_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "results": {
            "type": "array",
...
def build_stage1_improve_batch_intro(batch_items: List[Dict[str, Any]]) -> str:
    ids_text = "\n".join([f"- {x['item_id']}" for x in batch_items])
    return (
        "你将执行 stage1 improve 情感标注。\n"
        "任务目标：模拟人类在 1–2 秒内扫视图片时的直觉情绪判断（Stage1 / System-1）。\n"
        "你会收到一批 item。每个 item 的顺序都是：item_id 文本标记 -> 图片 -> 该图对应的 5 条补充信息。\n"
        "【顺序要求】先看图片形成第一眼直觉；再阅读该 item 的 5 条补充信息与软触发器做校正。\n"
        "注意：补充信息/触发器都是概率倾向，不是硬规则；若与图片直觉冲突，以图片直觉为准。\n\n"
        f"{SOFT_TRIGGER_TEXT}\n\n"
        f"{PREVENT_DEFAULT_TEXT}\n\n"
        "【观看条件】\n"
        "- 不深入分析\n"
        "- 不解释原因\n"
        "- 不做象征/叙事/文化解读\n\n"
...
def run_one_batch(client, types_module, registry: Dict[str, Any], batch_items: List[Dict[str, Any]], model: str, retry_sleep: float, max_retries: int) -> Dict[str, Any]:
    intro = build_stage1_improve_batch_intro(batch_items)
    ...
    for item in batch_items:
        ...
        cues_text = json.dumps(item.get("cues_top5") or {}, ensure_ascii=False) if item.get("cues_top5") else "(无)"
        contents.append(types_module.Part.from_text(text=(
            f"item_id: {item_id}\n"
            "下面是该图片对应的 5 条补充信息（全局感知），用于轻微校正：\n"
            f"{cues_text}\n"
            "请仅在最终 JSON 中给出该 item 的标签，不要单独解释。"
        )))
        ...
        try:
            response = client.models.generate_content(
                ...
                config={
                    ...
                    "response_json_schema": STAGE1_IMPROVE_BATCH_RESPONSE_JSON_SCHEMA,
                    "temperature": 0,
                },
            )
                    ...
                    for item in batch_items:
                        ...
                        if label in LABEL_SET:
                            result_map[iid] = {
                                ...
                                "has_cues": bool(item.get("has_cues")),
            ...
            for item in batch_items:
                ...
                result_map[iid] = {
                    ...
                    "meta": {
                        ...
                        "cues_top5": item.get("cues_top5") or {},
                    },
                }
                done_ids.add(iid)
            atomic_write_json(
...
# =========================================================
def parse_args():
    ap = argparse.ArgumentParser(description="Gemini 2.5 Pro Stage1 improve batch mode")
    ...
    ap.add_argument("--legacy_result_json", default=str(LEGACY_RESULT_JSON_DEFAULT), help="已有 stage1 improved 结果 json，用于跳过已完成样本")
...
def main():
    ...
    print("[all done] stage1 improve batch mode finished.")
