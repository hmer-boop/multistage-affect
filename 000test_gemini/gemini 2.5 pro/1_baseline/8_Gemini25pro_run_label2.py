# -*- coding: utf-8 -*-
"""
8_Gemini25pro_run_label2.py —— Gemini 2.5 Pro Stage2 单阶段标注（并发提速版）

用途：
1. 将原 GPT baseline 的 label2 迁移到 Gemini 2.5 Pro。
2. 输出结构与分 stage 目录一致：当前脚本只写入
   000test_gemini/gemini 2.5 pro/1_baseline/result Gemini_stage2/
...
输出（当前脚本目录下的独立 stage2 文件夹）：
- result Gemini_stage2/results_stage2.txt
- result Gemini_stage2/results_stage2.json
- result Gemini_stage2/missing_report_stage2.json
- result Gemini_stage2/results_stage2_round2.txt
- result Gemini_stage2/results_stage2_round2.json
- result Gemini_stage2/missing_report_stage2_round2.json
"""
...
# worker
# =========================================================
def worker_stage2(item: Dict, args, api_key: str, types_module):
...
# =========================================================
def parse_args():
    ap = argparse.ArgumentParser(description="Gemini 2.5 Pro Stage2 labeling with TXT/JSON dual save")
...
def run_one_dataset(
    ...
    if not todo:
        ...
        def submit_with_retry(executor, item):
            return executor.submit(worker_stage2, item, args, api_key, types_module)
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as ex:
            ...
            while future_to_ctx:
                for fut in as_completed(list(future_to_ctx.keys())):
                    ...
                    try:
                        res = fut.result()
                        existing[res["item_id"]] = res["stage2"]
                        new_count += 1

                        print(
                            f"[stage2:{dataset_name}] {new_count}/{total_todo} | {res['item_id']} -> 原始:{res['raw_result']} | 归一化:{res['stage2']}"
                        )
