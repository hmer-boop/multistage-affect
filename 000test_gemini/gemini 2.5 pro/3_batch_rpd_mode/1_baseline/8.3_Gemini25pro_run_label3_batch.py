# -*- coding: utf-8 -*-
"""
8.3_Gemini25pro_run_label3_batch.py

Gemini 2.5 Pro Stage3 baseline 批处理版（RPD 优化）。

设计目标
1) 保留原 stage3 baseline 的任务定义：图像 + title_en/caption_en，输出 8 类单标签。
...
5) 自动读取旧单图版 stage3 结果目录，已完成样本默认跳过不重跑。
6) 旧结果会直接合并复制到本次 batch 输出目录的 merged 结果文件中，便于后续只对这一套文件与金标对照。
7) 最终输出仍保持与旧脚本兼容的 merged 结果文件：
   - results_stage3.txt / json / missing_report_stage3.json
   - results_stage3_round2.txt / json / missing_report_stage3_round2.json

建议放置目录：
PythonProject/000test_gemini/gemini 2.5 pro/3_batch_rpd_mode/1_baseline/
"""
...
TXT_OUT = OUT_DIR / "results_stage3.txt"
JSON_OUT = OUT_DIR / "results_stage3.json"
REPORT_PATH = OUT_DIR / "missing_report_stage3.json"

TXT_OUT_ROUND2 = OUT_DIR / "results_stage3_round2.txt"
JSON_OUT_ROUND2 = OUT_DIR / "results_stage3_round2.json"
REPORT_PATH_ROUND2 = OUT_DIR / "missing_report_stage3_round2.json"

LEGACY_RESULT_DIR_DEFAULT = BASE_DIR.parent.parent / "1_baseline" / "result Gemini_stage3"
LEGACY_TXT_OUT = LEGACY_RESULT_DIR_DEFAULT / "results_stage3.txt"
LEGACY_TXT_OUT_ROUND2 = LEGACY_RESULT_DIR_DEFAULT / "results_stage3_round2.txt"

BATCH_OUTPUT_BASE_DIR = OUT_DIR / "base" / "batch_outputs"
BATCH_OUTPUT_ROUND2_DIR = OUT_DIR / "round2" / "batch_outputs"

MASTER_BASE_PATH = MANIFEST_MASTER_DIR / "stage3_baseline_base_master.json"
MASTER_ROUND2_PATH = MANIFEST_MASTER_DIR / "stage3_baseline_round2_master.json"
BATCH_BASE_DIR = MANIFEST_BATCH_DIR / "stage3_baseline_base"
BATCH_ROUND2_DIR = MANIFEST_BATCH_DIR / "stage3_baseline_round2"
...
for p in [
    ...
    MANIFEST_BATCH_DIR,
    OUT_DIR,
    BATCH_OUTPUT_BASE_DIR,
    BATCH_OUTPUT_ROUND2_DIR,
    BATCH_BASE_DIR,
    BATCH_ROUND2_DIR,
]:
...
def read_txt_index(txt_path: Path) -> Dict[str, str]:
    ...
    with txt_path.open("r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if not header_seen:
                header_seen = True
                if col1 == "item_id" and col2 == "stage3":
                    continue
            if col1:
                idx[col1] = col2 or ""
    return idx
...
def write_json_results(json_path: Path, data_map: Dict[str, str]):
    rows = [{"item_id": iid, "stage3": (lab or None)} for iid, lab in sorted(data_map.items())]
...
STAGE3_BATCH_RESPONSE_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "results": {
            "type": "array",
...
def build_stage3_batch_intro(batch_items: List[Dict[str, Any]]) -> str:
    ids = [str(x["item_id"]) for x in batch_items]
    ids_text = "\n".join([f"- {iid}" for iid in ids])
    return (
        "你将执行 stage3 baseline 情感标注。\n"
        "你会收到一批图片。每个图片前面都有一个 item_id 文本块，文本块中可能包含英文标题和英文描述，紧接着就是该图片。\n"
        "你的任务是：结合图像内容与给定英文标题/英文描述，预测该作品的情感。\n"
...
def run_one_batch(client, types_module, registry: Dict[str, Any], batch_items: List[Dict[str, Any]], model: str, retry_sleep: float, max_retries: int) -> Dict[str, Any]:
    intro = build_stage3_batch_intro(batch_items)
    ...
    for item in batch_items:
        ...
        title_en = safe_text(item.get("title_en"))
        caption_en = safe_text(item.get("caption_en"))
        item_text = (
            f"item_id: {item_id}\n"
            f"title_en: {title_en}\n"
            f"caption_en: {caption_en}"
        )
        contents.append(types_module.Part.from_text(text=item_text))
        ...
        try:
            response = client.models.generate_content(
                ...
                config={
                    ...
                    "response_json_schema": STAGE3_BATCH_RESPONSE_JSON_SCHEMA,
                    "temperature": 0,
                },
            )
...
def parse_args():
    ap = argparse.ArgumentParser(description="Gemini 2.5 Pro Stage3 baseline batch mode")
    ...
    ap.add_argument("--legacy_result_dir", default=str(LEGACY_RESULT_DIR_DEFAULT), help="旧单图 stage3 结果目录，用于跳过已完成样本")
    ap.add_argument("--ignore_legacy_results", action="store_true", help="不读取旧单图结果目录")
...
def main():
    ...
    legacy_txt_base = None if args.ignore_legacy_results else (legacy_result_dir / "results_stage3.txt")
    legacy_txt_round2 = None if args.ignore_legacy_results else (legacy_result_dir / "results_stage3_round2.txt")
    ...
    print("[all done] stage3 baseline batch mode finished.")
