# -*- coding: utf-8 -*-
"""
8.2_Gemini25pro_run_label2_batch.py

Gemini 2.5 Pro Stage2 baseline 批处理版（RPD 优化）。

设计目标
1) 保留原 stage2 baseline 的任务定义：只看图像，输出 8 类单标签。
...
5) 自动读取旧单图版 stage2 结果目录，已完成样本默认跳过不重跑。
6) 旧结果会直接合并复制到本次 batch 输出目录的 merged 结果文件中，便于后续只对这一套文件与金标对照。
7) 最终输出仍保持与旧脚本兼容的 merged 结果文件：
   - results_stage2.txt / json / missing_report_stage2.json
   - results_stage2_round2.txt / json / missing_report_stage2_round2.json
...
LEGACY_RESULT_DIR_DEFAULT = BASE_DIR.parent.parent / "1_baseline" / "result Gemini_stage2"
LEGACY_TXT_OUT = LEGACY_RESULT_DIR_DEFAULT / "results_stage2.txt"
LEGACY_TXT_OUT_ROUND2 = LEGACY_RESULT_DIR_DEFAULT / "results_stage2_round2.txt"

BATCH_OUTPUT_BASE_DIR = OUT_DIR / "base" / "batch_outputs"
BATCH_OUTPUT_ROUND2_DIR = OUT_DIR / "round2" / "batch_outputs"

MASTER_BASE_PATH = MANIFEST_MASTER_DIR / "stage2_baseline_base_master.json"
MASTER_ROUND2_PATH = MANIFEST_MASTER_DIR / "stage2_baseline_round2_master.json"
BATCH_BASE_DIR = MANIFEST_BATCH_DIR / "stage2_baseline_base"
BATCH_ROUND2_DIR = MANIFEST_BATCH_DIR / "stage2_baseline_round2"
...
def read_txt_index(txt_path: Path) -> Dict[str, str]:
    ...
    with txt_path.open("r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            if not header_seen:
                header_seen = True
                if col1 == "item_id" and col2 == "stage2":
                    continue
            if col1:
                idx[col1] = col2 or ""
    return idx
...
def write_json_results(json_path: Path, data_map: Dict[str, str]):
    rows = [{"item_id": iid, "stage2": (lab or None)} for iid, lab in sorted(data_map.items())]
...
# Gemini 批请求
# =========================================================
STAGE2_BATCH_RESPONSE_JSON_SCHEMA = {
    "type": "object",
    "properties": {
        "results": {
            "type": "array",
...
def build_stage2_batch_intro(batch_items: List[Dict[str, Any]]) -> str:
    ids = [str(x["item_id"]) for x in batch_items]
    ids_text = "\n".join([f"- {iid}" for iid in ids])
    return (
        "你将执行 stage2 baseline 情感标注。\n"
        "你会收到一批图片。每个图片前面都有一个 item_id 文本标记，紧接着就是该图片。\n"
        "你的任务是：模拟人在持续注视这幅艺术作品后的深层情感体验。\n"
...
def run_one_batch(client, types_module, registry: Dict[str, Any], batch_items: List[Dict[str, Any]], model: str, retry_sleep: float, max_retries: int) -> Dict[str, Any]:
    intro = build_stage2_batch_intro(batch_items)
        ...
        try:
            response = client.models.generate_content(
                ...
                config={
                    ...
                    "response_json_schema": STAGE2_BATCH_RESPONSE_JSON_SCHEMA,
                    "temperature": 0,
                },
            )
...
# =========================================================
def parse_args():
    ap = argparse.ArgumentParser(description="Gemini 2.5 Pro Stage2 baseline batch mode")
    ...
    ap.add_argument("--legacy_result_dir", default=str(LEGACY_RESULT_DIR_DEFAULT), help="旧单图 stage2 结果目录，用于跳过已完成样本")
...
def main():
    ...
    legacy_txt_base = None if args.ignore_legacy_results else (legacy_result_dir / "results_stage2.txt")
    legacy_txt_round2 = None if args.ignore_legacy_results else (legacy_result_dir / "results_stage2_round2.txt")
    ...
    print("[all done] stage2 baseline batch mode finished.")
