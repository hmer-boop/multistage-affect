...
7) 自动读取旧单图版 stage1 improve 结果，已完成样本默认跳过不重跑。
8) 旧结果会直接合并复制到本次 batch 输出目录的 merged 结果文件中，便于后续只看这一套文件与金标对照。
...
def parse_args():
    ...
    ap.add_argument("--legacy_result_json", default=str(LEGACY_RESULT_JSON_DEFAULT), help="旧单图 stage1 improve 结果 json，用于跳过已完成样本")
