...
TXT_OUT = OUT_DIR / f"results_stage2_{MODEL_TAG}.txt"
JSON_OUT = OUT_DIR / f"results_stage2_{MODEL_TAG}.json"
REPORT_PATH = OUT_DIR / f"missing_report_stage2_{MODEL_TAG}.json"
LOCK_PATH = OUT_DIR / f".results_stage2_{MODEL_TAG}.lock"

TXT_OUT_ROUND2 = OUT_DIR / f"results_stage2_{MODEL_TAG}_round2.txt"
JSON_OUT_ROUND2 = OUT_DIR / f"results_stage2_{MODEL_TAG}_round2.json"
REPORT_PATH_ROUND2 = OUT_DIR / f"missing_report_stage2_{MODEL_TAG}_round2.json"
LOCK_PATH_ROUND2 = OUT_DIR / f".results_stage2_{MODEL_TAG}_round2.lock"
...
def call_vision(client: OpenAI, model: str, instruction: str, image_data_url: str, request_timeout: float = 60.0) -> str:
...
def worker_stage2(item: Dict, client: OpenAI, model: str, max_side: int, quality: int, rate_delay: float, request_timeout: float = 60.0) -> Dict:
...
def run_one_dataset(items: List[Dict], txt_out: Path, json_out: Path, report_path: Path, lock_path: Path, args, dataset_name: str):
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as ex:
        futures = [
            ex.submit(worker_stage2, it, client, args.model, args.max_side, args.quality, args.rate_delay, args.request_timeout)
            for it in todo
        ]
