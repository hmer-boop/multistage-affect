...
PROMPT_VERSION = "stage1_glance_v1_qwen8b_improve"
...
TXT_OUT = OUT_DIR / "results_stage1_qwen8b_improve.txt"
JSON_OUT = OUT_DIR / "results_stage1_qwen8b_improve.json"
STATE_JSON = OUT_DIR / "results_stage1_qwen8b_improve_state.json"
REPORT_PATH = OUT_DIR / "missing_report_stage1_qwen8b_improve.json"
LOCK_PATH = OUT_DIR / ".results_stage1_qwen8b_improve.lock"
...
def parse_args():
    ...
    ap.add_argument("--model", default="qwen3-vl-8b-instruct", help="DashScope 视觉模型，如 qwen3-vl-8b-instruct")
    ...
    ap.add_argument("--max_workers", type=int, default=4, help="并发线程数（默认4）")
    ap.add_argument("--rate_delay", type=float, default=0.0, help="每次请求后的延时秒数（默认0）")
    ap.add_argument("--request_timeout", type=float, default=45.0, help="单次API请求超时（默认45s）")
    ap.add_argument("--future_timeout", type=float, default=180.0, help="每个样本总体等待上限（默认180s）")
