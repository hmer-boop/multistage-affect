...
def merge_id_maps(*maps: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    后传入的 map 覆盖前面的同 ID 记录，和 baseline 现有 9.6 口径保持一致。
    """
...
def get_eval_ids(
    ...
    """
    CRR 只评价 Stage II -> Stage III，因此要求：
...
def evaluate_context_regression_rate(
    ...
    含义：
    - Stage2 正确：仅基于图像的延长观察阶段命中 stage2 金标；
    - Stage3 错误：引入标题/描述文本后没有命中 stage3 金标；
    - 该指标衡量文本语境引入后，将原本正确判断改错的比例。越低越好。
    """
    ...
    summary = {
        ...
        "stage2_acc_on_eval_ids": stage2_correct_count / total if total else 0.0,
        "stage3_acc_on_eval_ids": stage3_correct_count / total if total else 0.0,
...
def main() -> None:
    parser = build_common_parser("9.8 Context Regression Rate (CRR)", include_output_csv=True)
    ...
    summary, rows = evaluate_context_regression_rate(
        gold_map=gold_map,
