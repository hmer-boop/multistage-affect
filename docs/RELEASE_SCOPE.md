...
- Low-resolution 512 px Artmajeur image files for Artmajeur-linked items in the 760-item subset, accompanied by an image manifest and attribution/provenance tables.
...
- A data provenance note explaining which image files are included and why non-Artmajeur image files are omitted from the review-stage repository.
...
- Image files from source sites whose reuse permission is still unresolved.
...
The goal is to let reviewers inspect and reproduce the experimental logic without exposing the complete private dataset during review. The full dataset and additional artifacts can be released after acceptance, subject to copyright, consent, and journal policy constraints.
