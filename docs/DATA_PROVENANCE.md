...
- `data/release_subset/labels/gold_label.json`: gold labels for the 760-item review subset.
- `data/release_subset/metadata/items_public.jsonl`: sanitized metadata for the same subset.
- `data/release_subset/images/artmajeur/`: included ArtMajeur image files.
- `data/release_subset/images/artmajeur_images_manifest.csv`: item-to-image manifest for the included ArtMajeur files, including repository image links in `github_image_url`.
- `data/release_subset/provenance/image_provenance_review.xlsx`: reviewer-facing provenance workbook sorted for lookup. The `github_image_url` column is populated for released ArtMajeur image files and left blank for items provided through source/provenance lookup only.
- `data/release_subset/provenance/image_provenance_review.csv`: CSV version of the same reviewer-facing provenance table.
- `data/release_subset/provenance/image_sources_manifest.csv`: item-level provenance worksheet, including repository image links where applicable.
- `data/release_subset/reports/provenance_summary.md`: current provenance-audit summary.
- `examples/cue_sample/`: representative stage-level cue examples.
- `examples/demo_predictions/`: review-demo prediction files that can be used to recompute metrics without external model calls.

## URL Fields

In `data/release_subset/provenance/image_sources_manifest.csv`, `source_collection_url` records an artist page, collection page, or source-site scope. This is not necessarily a link to the exact artwork.
