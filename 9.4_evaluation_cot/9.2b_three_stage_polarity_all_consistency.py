...
def is_stage_polarity_exact_match(gold_labels, pred_labels) -> bool:
    gold_pols = labels_to_polarity_set(gold_labels)
    pred_pols = labels_to_polarity_set(pred_labels)
    return bool(gold_pols) and bool(pred_pols) and gold_pols == pred_pols


def evaluate_three_stage_polarity_exact_match_pred_driven(gold_map, pred1, pred2, pred3):
    ...
    for item_id in common_pred_ids:
        ...
        g1 = to_list_labels(extract_stage_label(grec, 'stage1'), normalize_text)
        g2 = to_list_labels(extract_stage_label(grec, 'stage2'), normalize_text)
        g3 = to_list_labels(extract_stage_label(grec, 'stage3'), normalize_text)
        if not labels_to_polarity_set(g1) or not labels_to_polarity_set(g2) or not labels_to_polarity_set(g3):
            continue
        p1 = to_list_labels(extract_stage_label(pred1[item_id], 'stage1'), normalize_text)
        p2 = to_list_labels(extract_stage_label(pred2[item_id], 'stage2'), normalize_text)
        p3 = to_list_labels(extract_stage_label(pred3[item_id], 'stage3'), normalize_text)
        total += 1
        if (
            is_stage_polarity_exact_match(g1, p1)
            and is_stage_polarity_exact_match(g2, p2)
            and is_stage_polarity_exact_match(g3, p3)
        ):
            correct += 1
...
def main():
    parser = build_common_parser('9.2b Three-stage Polarity All Consistency')
    ...
    total, rate = evaluate_three_stage_polarity_exact_match_pred_driven(gold_map, pred1, pred2, pred3)
    print(f'three_stage_polarity_exact_match_rate: {rate:.4f} | 分母={total}')
